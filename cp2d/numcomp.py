    ...
    if iniFile:
        ...
        if "numcomp" in config:
            if config["numcomp"].getint("numCpus") == None:
                ...
                print(
                    "Missing 'numCpus' value in config file. Trying to deduce from system.",
                    file=sys.stderr,
                )
                try:
                    ...
                    print(
                        "Failed to get number of cpus. Going on with default value, fingers crossed.",
                        file=sys.stderr,
                    )
            else:
            ...
            if config["numcomp"].getint("buffer") == None:
                print(
                    "Missing 'buffer' value in config file. Going on with default value.",
                    file=sys.stderr,
                )
                if (
                    "elaboration" in config
                    and "workers" in config["elaboration"]
                    and config["elaboration"].getint("workers") < numCpus
                ):
                    ...
                    print(
                        "Failed to deduce 'buffer' size. Going on with default value.",
                        file=sys.stderr,
                    )
            else:
            ...
            if config["numcomp"].getint("limit") == None:
                ...
                print(
                    "Missing 'limit' value in config file. Going on with default value.",
                    file=sys.stderr,
                )
            else:
            ...
            if config["numcomp"].getfloat("timeout") == None:
                ...
                print(
                    "Missing 'timeout' value in config file. Going on with default value.",
                    file=sys.stderr,
                )
            else:
            ...
            if config["numcomp"].getfloat("cpuThreshold") == None:
                ...
                print(
                    "Missing 'cpuThreshold' value in config file. Going on with default value.",
                    file=sys.stderr,
                )
            else:
            ...
            print(
                "Missing 'numcomp' section in config file. Going on with default values.",
                file=sys.stderr,
            )
    else:
        ...
        while True:
            ...
            try:
                ...
                for process in psutil.process_iter(
                    ["username", "cpu_percent", "exe"], "0.0"
                ):
                    # to avoid shrinking from shells
                    if (
                        process.info["username"] in nomi
                        and float(process.info["cpu_percent"]) > cpuThreshold
                    ):
                        count += 1
                    if not my_proc and "fragprob" in process.info["exe"]:
                        my_proc = [
                            psutil.Process(A.id)
                            for A in process.threads()
                            if A.id != process.pid
                        ]
                        my_proc = [
                            p
                            for p in my_proc
                            if float(process.info["cpu_percent"]) > cpuThreshold
                        ]
                ...
                if count > tmp_buff and limit > tmp_buff:
                    target = count - tmp_buff if count < limit else limit - tmp_buff
                else:
                    target = 0
                my_proc = old_r[:] + old_s[:]
                ...
                logger.info(f"Now {len(old_s)+len(old_r)} processes left running")
...
def calprob(
    ...
    if len(glob(os.path.join(resultsDir, outName + "_*.res"))):
        return
    if len(glob(os.path.join(resultsDir, "tmpres_*.res"))) and not len(
        glob(os.path.join(resultsDir, "tmpres_uo_*.res"))
    ):
    ...
    try:
        ...
        p = [
            mp.Process(target=process_controller, args=()),
        ]
        # p[0].start()
            ...
            if rlist and sys.stdin.isatty():
                ...
                if inp == "t":
                    ...
                    try:
                        ...
                        print(
                            "Error retrieving processes info\n", process, process.info
                        )
...
def prob_compute(
    ...
    """
    args = [
        os.path.join(el_path, "../bin/fragprob"),
        "-f",
        os.path.join(dataDir, "seq"),
        "-o",
        resultsDir,
        "-F",
        str(fragment),
...
if __name__ == "__main__":
    print(
        "This script is intended only as a support for workflow.py. Use that.",
        file=sys.stderr,
    )
    exit(1)
