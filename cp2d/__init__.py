...
def main():
    parser = argparse.ArgumentParser(
        prog="CP2D",
        description="Authorship attribution using the CP2D  approach. For each subcommand help, use <subcommand> -h.",
        epilog="Giulio Tani Raffaelli, 2023",
    )
    ...
    run_parser = subparsers.add_parser(
        "attribute",
        help="Attributes the the documents in a given corpus to their authors.",
    )
    run_parser.add_argument(
        "database",
        type=str,
        help="Path to the database folder (e.g. the output folder chosen with data preprocessing subcommand 'prepare').",
        metavar="corpus path",
    )

    group_run1 = run_parser.add_mutually_exclusive_group()
    group_run1.add_argument(
        "-n",
        type=int,
        dest="ngramSize",
        metavar="N",
    ...
    group_run1.add_argument(
        "-U",
        action="store_true",
        help="Keep underscores and punctuation when using dictionary method (incompatible with 'z' and 'n')",
        dest="keepNonAlfabetical",
    )
    group_run1.add_argument(
        "-z",
        action="store_true",
        help="Use LZ77 dictionaries (incompatible with 'n' and 'U')",
        dest="LZ77",
    )
    run_parser.add_argument(
        "-w",
        type=int,
        help="Length of the LZ77 window (default: use full document)."
        ...
        "lengths across the documents. Ignored if '-z' not provided.",
        metavar="length",
        default=0,
        dest="window",
    )
    run_parser.add_argument(
        "-f",
        type=int,
        help="Size of the document fragments (default: use full documents).",
        metavar="size",
        default=0,
        dest="fragment",
    )
    run_parser.add_argument(
        "-F",
        type=int,
        help="Size of the author slices (default: use full authors). Usually this length should be at least ten times the fragments length.",
        metavar="size",
        default=0,
        dest="authorLength",
    )
    ...
    run_parser.add_argument(
        "-c",
        type=str,
        help="Path to the config file if not using a global configuration.",
        metavar="path",
        dest="configFile",
        default="",
    )
    run_parser.add_argument(
        "-p",
        type=str,
        help="Path to the file storing P0.",
        metavar="path",
        dest="P0file",
    )
    run_parser.add_argument(
        ...
        dest="delta",
        default=1,
        help="Values of delta for attribution.",
    )
    group_run2 = run_parser.add_mutually_exclusive_group()
    group_run2.add_argument(
        "-l",
        type=int,
        nargs="?",
        const=1,
    ...
    group_run4.add_argument(
        "-o",
        action="store_false",
        default=False,
        dest="overwriteOutputs",
    ...
    run_parser.add_argument(
        "-a",
        type=str,
        help="Path to the author association file.",
        metavar="path",
        dest="association",
        default={},
    )
    ...
    prepare_parser.add_argument(
        ...
        type=str,
        dest="inFolder",
        help="Input folder containing the corpus.",
        default="",
        metavar="path",
    )
    prepare_parser.add_argument(
        ...
        type=str,
        dest="outFolder",
        help="Output folder for the processed corpus.",
        default="",
        metavar="path",
    )
    prepare_parser.add_argument(
    ...
    group_conf.add_argument(
        ...
        "-o",
        type=str,
        help="Output folder for the config file.",
        default="",
        metavar="path",
        dest="outFolder",
    )
    config_parser.set_defaults(func=support.build_config, globalDir=os.getcwd())
    ...
    group_shuff.add_argument(
        "--preserve",
        "-p",
        action="store_true",
        help="Preserve the distribution of books per author.",
        default=None,
        dest="preserveDist",
    )
    group_shuff.add_argument(
        ...
        "-n",
        action="store_false",
        help="Do not preserve the distribution of books per author.",
        default=None,
        dest="preserveDist",
    )
