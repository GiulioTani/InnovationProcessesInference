...
def output_file_names(dataDir, filename, extension):
    outputFileName = os.path.join(dataDir, extension, f"{filename}." + extension)
    outputFileTmpName = os.path.join(dataDir, extension, f"tmp{filename}." + extension)
...
def selectDelta(ret):
    oldres_df = pd.DataFrame(
        [
            {"delta": d, "FNN": v["all"]["FNN"], "MR": v["all"]["MR"]}
...
def preproc_pro(ab):
    ...
    if _Q_association:
        ...
        def goodness(a):
            return (a not in _Q_association and a not in _Q_excluded) or (
        ...
        def goodness(a):
            return a not in _Q_excluded

    if not _Q_which_slice_is_None:
        s = _Q_which_slice[ab]
        lcid = {
            a: (_Q_apt[a][0][s] if ab[0] != 0 and s in _Q_apt[a][0] else _Q_apt[a][1])
            for a in _Q_apt
        }
        ...
        if _Q_association:
            ...
            def mycid(a):
                return a if a not in _Q_association else _Q_association[a]
        ...
        for ab in attributions:
            ...
            for attributionType in attributions[tab][nowDelta]:
                if attributionType == "FRA":
                    toppers.update(
                        {
                            "FRA_" + k: attributions[ab][nowDelta]["FRA"][i]
                            for i, k in enumerate(["MR", "TMR", "WMR"])
                        }
                    )
                else:
                    toppers[attributionType] = attributions[ab][nowDelta][
                        attributionType
                    ][0][0]
            ret[nowDelta].append(toppers)
    ret_df = {
        delta: pd.DataFrame(ret[delta]).set_index(["A", "B", "S"]).sort_index()
        for delta in ret
    }
...
def return_dict(
    ...
    nonAttri: tp.Sequence[int],
    allowPartial: bool,
    authors: tp.Sequence,
    sliceSeparated: tp.Optional[int] = None,
    goodSlices: tp.Sequence[int] = [],
) -> tp.Tuple[
    tp.Mapping[float, tp.Mapping[str, tp.Mapping[str, float]]],
    tp.Optional[tp.List[tp.Mapping[float, tp.Mapping[str, tp.Mapping[str, float]]]]],
]:
    """From the proposed attributions compute base statistics.
    ...
    for tab in attributions:
        break
    if nonAttri:
        notExcludedAuthor = np.array(
            [(a not in nonAttri) and (a > 0) for a in authors], dtype=bool
        )  # not attributed

    authorOrder = {a: i for i, a in enumerate(authors)}  # author order
    authorBooksInSlice = np.zeros(
        (len(authors), sliceSeparated)
    )  # author books in slice
    authorFragmentsInSlice = np.zeros(
        (len(authors), sliceSeparated)
    ...
    for ab in attributions:
        if ab[0]:
            if goodSlices and _Q_which_slice[ab] not in goodSlices:
                continue
            my_slice = (
                goodSlices.index(slices[ab])
                if goodSlices and sliceSeparated > 1
                else slices[ab]
            )
            authorBooksInSlice[authorOrder[ab[0]], my_slice] += 1
            authorFragmentsInSlice[authorOrder[ab[0]], my_slice] += fra.loc[ab, "fn"]
    for nowDelta in attributions[tab]:
        authorCorrectForSliceType = {
            typ: np.zeros((len(authors), sliceSeparated))
            for typ in attributions[tab][nowDelta]
        }
        authorAssignedForSliceType = {
            typ: np.zeros((len(authors), sliceSeparated))
            for typ in attributions[tab][nowDelta]
        }
        for ab in attributions:
            ...
            for attributionType in attributions[tab][nowDelta]:
                if attributionType == "FRA":
                    authorCorrectForSliceType["FRA"][
                        authorOrder[ab[0]], my_slice
                    ] += attributions[ab][nowDelta]["FRA"][0]
                else:
                    if ab[0] in attributions[ab][nowDelta][attributionType][0][0]:
                        authorCorrectForSliceType[attributionType][
                            authorOrder[ab[0]], my_slice
                        ] += fractyp(
                            1 / len(attributions[ab][nowDelta][attributionType][0][0])
                        )
                    if (
                        allowPartial
                        or len(attributions[ab][nowDelta][attributionType][0][0]) == 1
                    ):
                        for a in attributions[ab][nowDelta][attributionType][0][0]:
                            try:
                                authorAssignedForSliceType[attributionType][
                                    authorOrder[a], my_slice
                                ] += 1
                            except KeyError:
                                pass
        ret[nowDelta] = {"all": {}}
        ...
        for attributionType in authorCorrectForSliceType:
            ...
            ret[nowDelta]["all"][attributionType] = StatsFromAuthorsSLice(
                tmpAuthorCorrect,
                tmpAuthorAssigned,
                authorBooksTotal,
                authorFragmentsTotal,
                attributionType,
            )
            if nonAttri:
                ret[nowDelta]["exc"][attributionType] = StatsFromAuthorsSLice(
                    tmpAuthorCorrect,
                    tmpAuthorAssigned,
                    authorBooksTotal,
                    authorFragmentsTotal,
        ...
        if sliceSeparated > 1:
            ...
            for s in range(sliceSeparated):
                ...
                for attributionType in authorCorrectForSliceType:
                    slSep[nowDelta][s]["all"][attributionType] = StatsFromAuthorsSLice(
                        authorCorrectForSliceType[attributionType][:, s],
                        authorAssignedForSliceType[attributionType][:, s],
                        authorBooksInSlice[:, s],
                        authorFragmentsInSlice[:, s],
                        attributionType,
                    )
                    if nonAttri:
                        slSep[nowDelta][s]["exc"][attributionType] = (
                            StatsFromAuthorsSLice(
                                authorCorrectForSliceType[attributionType][:, s],
                                authorAssignedForSliceType[attributionType][:, s],
                                authorBooksInSlice[:, s],
                                authorFragmentsInSlice[:, s],
...
def StatsFromAuthorsSLice(
    tmpAuthorCorrect,
    tmpAuthorAssigned,
    authorBooksTotal,
    authorFragmentsTotal,
    attributionType,
    notExcludedAuthor=slice(None),
):
    if attributionType == "FRA":
        return np.sum(tmpAuthorCorrect[notExcludedAuthor]) / np.sum(
            authorFragmentsTotal[notExcludedAuthor]
        )
    else:
        return StatsFromType(
            authorBooksTotal[notExcludedAuthor],
            tmpAuthorCorrect[notExcludedAuthor],
            tmpAuthorAssigned[notExcludedAuthor],
        )
...
def StatsFromType(authorBooksTotal, authorCorrect, authorAssigned):
    ...
    return {
        "weigh": {
            "P": np.average(precision, weights=authorBooksTotal),
            "R": np.average(recall, weights=authorBooksTotal),
            "F": np.average(F1, weights=authorBooksTotal),
        },
        "macro": {
            "P": np.average(precision, weights=mask2),
            "R": np.average(recall, weights=mask2),
            "F": np.average(F1, weights=mask2),
        },
    }
...
def assign_unknown(attributions, delta: float = None):
    ...
    for nowDelta in whichDeltas:
        for ab in attributions:
            if ab[0] <= 0:
                unk[nowDelta][str(ab)] = {
                    k: v for k, v in attributions[ab][nowDelta].items() if k != "FRA"
                }
    return unk
...
def add_results_line(
    ...
    res_dic = {
        "ret": ret,
        "exc": list(set(_Q_excluded)),
        "noa": list(set(nonAttri)),
        "asso": _Q_association,
        "part": allowPartial,
        "delta": logDelta,
        "unknowns": unk if saveUnk else {},
        "slices": goodSlices if goodSlices else [],
    }
    if F is not None:
        res_dic["F"] = F
    if A is not None:
...
def previous_results(
    dataDir, nonAttri, logDelta, allowPartial, allDeltas=False, DBDir=None
):
    res = {}
    unk = {}
    if os.path.isfile(os.path.join(dataDir, "results.json")):
        ...
        for line in previous_results:
            try:
                ...
                continue
            if {"part", "exc", "noa", "asso", "ret", "delta", "unknowns"} == set(
                res_dic.keys()
            ):
                if set(res_dic["exc"]) == set(_Q_excluded) and (
                    set(res_dic["noa"]) == set(nonAttri)
                    and res_dic["part"] == allowPartial
                    and res_dic["delta"] in logDelta
                    or allDeltas
                ):
                    if loadAssociation(res_dic["asso"], DBDir) == _Q_association:
                        ...
                        if res_dic["unknowns"]:
                            unk[res_dic["delta"]] = {
                                k: v for k, v in res_dic["unknowns"].items()
        ...
        for line in previous_results:
            ...
            if {"part", "exc", "noa", "asso", "ret", "delta", "unknowns"} == set(
                ...
                previous_settings.add(
                    (
                        frozenset(res_dic["exc"]),
                        tuple(sorted(loadAssociation(res_dic["asso"], DBDir).items())),
                    )
                )
...
def top_performers(_tarr, asso):
    ...
    ret = [
        [[asso[int(tarr[-1][0])]], int(tarr[-1][1])],
    ]
    for a in tarr[-2::-1, :]:
        if a[1] < ret[0][1]:
...
def print_margout(
    dataDir, attributions, nonAttri, logDelta, allowPartial, returnDict, nowDelta
):
    ...
    print_margout_head(
        set(_Q_excluded), set(nonAttri), _Q_association, logDelta, file=outpar
    )
    if nonAttri:
        outnop = open(os.path.join(dataDir, f"{baseMR}_nonAttri.txt"), "w")
        print_margout_head(
            set(_Q_excluded), set(nonAttri), _Q_association, logDelta, file=outnop
        )
    titles = {}
    for ab in sorted(attributions, key=lambda x: x[0] * 1e6 + x[1]):
        ...
        print_margout_book_line(ab, attributions[ab][nowDelta], fragnum(ab), outpar)
        if nonAttri and ab[0] not in nonAttri:
            print_margout_book_line(ab, attributions[ab][nowDelta], fragnum(ab), outnop)
    ...
    for author in sorted(titles.keys(), key=lambda x: len(titles[x]), reverse=True):
        ...
        for typ in attributions[tab][nowDelta]:
            ...
            for ab in attributions:
                if (
                    ab[0] == author
                    and (typ in attributions[ab][nowDelta])
                    and (ab[0] in attributions[ab][nowDelta][typ][0][0])
                ):
                    tmtyre += round(
                        1 / len(attributions[ab][nowDelta][typ][0][0]), 5 * allowPartial
                    )
            tmp[typ] = tmtyre
        aF = sum([fragnum(ab) for ab in attributions if ab[0] == author])
        print_margout_author_line(
            author,
            scores=tmp,
            numBooks=len(titles[author]),
            numFragments=aF,
            file=outpar,
        )
...
def print_margout_book_line(authorBook, attribution, fragmentNum, file):
    ...
    for att in ["FNN", "TOP", "WP"]:
        ...
        try:
            print(
                f"{att}: {res[0][0]}: {res[0][1]:.4} {f'{res[1][0][0]}: {res[1][1]:.4}' if len(res[1][0]) else '--'}",
                end="\t",
                file=file,
            )
        except IndexError:
    ...
    for att in ["MR", "TMR", "WMR"]:
        ...
        try:
            print(
                f"{att}: {res[0][0]}: {res[0][1]} {f'{res[1][0][0]}: {res[1][1]}' if len(res[1][0]) else '--'}",
                end="\t",
                file=file,
            )
        except IndexError:
...
def print_margout_summary_line(returnDict, file, which="all"):
    ...
    for r in returnDict[which].items():
        if r[0] == "FRA":
            ...
            print(
                r[0],
                *(f"{r[1]['weigh'][measure]*100:.4}%" for measure in ["P", "R", "F"]),
                *(f"{r[1]['macro'][measure]*100:.4}%" for measure in ["P", "R", "F"]),
                sep="\t",
                file=file,
            )
    print("\n\n", file=file)
...
def print_margout_author_line(author, scores, numBooks, numFragments, file):
    ...
    for typ in scores:
        if typ != "FRA":
            print(
                f"{typ}: {round(scores[typ],1)} {scores[typ]/numBooks*100:.4} %",
                end="\t",
                file=file,
            )
    print(f"FRA: {scores['FRA']} {scores['FRA']/numFragments*100:.4} %", file=file)
...
def check_sources(el_path):
    ...
    if os.path.getmtime(os.path.join(el_path, "bin/bookprob.o")) < os.path.getmtime(
        os.path.join(el_path, "src/bookprob.cpp")
    ):
        logger.warning(
    ...
    if os.path.getmtime(os.path.join(el_path, "bin/fragprob.o")) < os.path.getmtime(
        os.path.join(el_path, "src/fragprob.cpp")
    ):
        logger.warning(
            "Source newer than executable, actual normalization may be different"
        )
    ...
    if os.path.getmtime(
        os.path.join(el_path, "bin/authorSplitter.o")
    ) < os.path.getmtime(os.path.join(el_path, "src/authorSplitter.cpp")):
        logger.warning(
            "Source newer than executable, actual normalization may be different"
        )
        warnFlag = True
...
def check_parameters(param):
    param.register_checks(
        {
            o: lambda x: isinstance(x, (int, np.integer)) and x >= -1
            for o in ["window", "LZ77baseFragments"]
        }
    )
    param.register_checks(
        {
            o: lambda x: isinstance(x, (int, np.integer)) and x >= 0
            for o in ["fragment", "authorLength"]
        }
    )
    param.register_checks(
    ...
    param.register_checks(
        delta=lambda x: (np.isscalar(x) and np.isreal(x) and np.isfinite(x))
        or (not np.isscalar(x) and np.isreal(x).all() and np.isfinite(x).all())
    )
    param.register_checks(database=lambda x: os.path.isdir(x))
    param.register_checks(baseCorpusDir=lambda x: not x or os.path.isdir(x))
    param.register_checks(
        {o: lambda x: not x or os.path.isfile(x)} for o in ["configFile", "P0file"]
    )
    param.register_checks(
        {
            o: lambda x: type(x) == bool
            for o in [
                "LZ77",
                "retrieve",
                "overwriteOutputs",
    ...
    param.register_checks(
        suffix=lambda x: not x or (type(x) == str and "seq" not in x and "wnt" not in x)
    )
...
def seq_to_wnt(
    fragment: str, compression: bool, window: int, Encoding: str, translation
):
    if compression:
        fragment = LZ77dict(fragment, window, Encoding)

    clean = (
        ("" if fragment[0] == " " else " ")
        + fragment.translate(translation).replace("\u2026", "...")
        + ("" if fragment[-1] == " " else " ")
    )
...
def create_tmpfile_dir(dataDir, tmpKind):
    outputDir = os.path.join(dataDir, tmpKind)
...
def read_config(configPath=None, workers=4, Encoding="latin1"):
    if not configPath:
        ...
        # looks for the config file in this sort of "PATH"
            ...
            print(f"Can't find config file {configPath}", file=sys.stderr)
    ...
    if "elaboration" in config:
        if config["elaboration"].getint("workers") == None:
            ...
            if (
                "numcomp" in config
                and "numCpus" in config["numcomp"]
                and "buffer" in config["numcomp"]
            ):
                workers = config["numcomp"].getint("numCpus") - config[
                    "numcomp"
                ].getint("buffer")
            else:
                logger.warnng(
    ...
    if l > 0:
        ...
        if tf == folds:
            ...
            if abs(tf2 - folds) < abs(tf - folds):
                ...
                l += 1
            print(
                f"Too few documents for a nice {folds}-fold cross-validation.\n"
        ...
        print(
            f"Too few documents for {folds}-fold cross-validation.\n"
            f"Falling back to a leave-one-out",
            file=sys.stderr,
        )
        return 1
...
def dirnames(param):
    ...
    suffixPart = (
        param["suffix"]
        if param["suffix"]
        else "".join(map(chr, np.random.randint(65, 91, 5)))
    )
    basename = "-".join([methodPart, midpart, leaveNoutPart, suffixPart])

    authorPart = (
        "fullA" if param["authorLength"] == 0 else f"{param['authorLength']:06}BA"
    )

    fragmentPart = "infBF" if not param["fragment"] else f"{param['fragment']:06}BF"
...
def derive_translation(LZ77: bool = False, ngramSize: int = 0, keepUnder: bool = False):
    if LZ77:
        ...
        control = support.control + "".join(
            [chr(i) for i in range(int("a", 16), int("e", 16))]
        )
    ...
    if LZ77:
        ...
        translation = str.maketrans(
            support.nonAlfa + control + support.unicpunt,
