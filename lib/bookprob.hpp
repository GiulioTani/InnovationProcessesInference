...
namespace bookprob
{
    ...
    public:
         ...
         * @param pref The prefix, i.e. an unique name, for the book.
         */
        book(const std::vector<std::pair<hash_type, int>> &ord, const std::vector<int> &times, const glob_prob *inP0 = NULL, double Alpha = -100, double Theta = -100, std::string pref = "");
        ...
        const std::vector<int> &get_entry_t() const { return entry_t; }
    ...
    protected:
        ...
        std::vector<int> entry_t /** The the time at which the new words enter the book.*/;
