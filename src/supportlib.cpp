            ...
            if (inseq.is_open())
            {
                while (getline(inseq, line))
                {
                    ...
                    if (splitBook.empty())
                    {
                        ...
                        fragname.str("");
                        fragname << "A" << A << "B" << B << "F1";
                        tmp_short[B].push_back(std::make_unique<bp::book>(bp::book(std::vector<std::pair<bp::hash_type, int>>{std::make_pair(std::hash<std::string>{}(""), 0)}, std::vector<int>{}, &P0, -100, -100, fragname.str())));
                        continue;
                    }
                    if (F == 1)
                    {
